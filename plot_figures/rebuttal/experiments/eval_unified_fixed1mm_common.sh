...
# Shared runner for the four fixed-fixture 1 mm unified-policy evaluations.
...
RUN_DIR="$REPO_ROOT/train_dir/multiproblem/cotrain/multiproblem_4way_peg1mm_2026-08-09_16-22-10"
CHECKPOINT="$RUN_DIR/eval_snapshot_fixed1mm_20260809.pth"
TRAIN_OVERRIDES="$RUN_DIR/.hydra/overrides.yaml"
OUT_DIR="$REPO_ROOT/plot_figures/rebuttal/experiments/outputs/unified_fixed1mm_checkpoints"
OUTPUT_JSON="$OUT_DIR/${EVAL_TAG}_fixed_job${SLURM_JOB_ID}.json"
DETAIL_LOG="$OUT_DIR/${EVAL_TAG}_fixed_job${SLURM_JOB_ID}.log"

mkdir -p "$OUT_DIR" "$REPO_ROOT/plot_figures/rebuttal/experiments/logs"
cd "$REPO_ROOT"
source .venv_isaacsim/bin/activate
export OMNI_KIT_ACCEPT_EULA=YES
export OMNI_KIT_CACHE_PATH="/tmp/${USER}_ov_cache_uni1mm_${EVAL_TAG}_${SLURM_JOB_ID}"
...
grep -E "problem=|all envs|no early drop|terminations:|Traceback|Error|=> wrote" "$DETAIL_LOG" || true
