...
def main() -> None:
    ...
    time_s = data["time_s"]
    pred_pos = data.get("predicted_object_pos")
    pred_quat = data.get("predicted_object_quat_xyzw")
    n = int(q.shape[0])
    ...
    server.scene.add_frame("/robot/cmd", position=(0.0, 0.8, 0.0), show_axes=False)
    robot = ViserUrdf(server, robot_urdf_path, root_node_name="/robot/state")
    robot_cmd = ViserUrdf(server, robot_urdf_path, root_node_name="/robot/cmd", mesh_color_override=(0, 0, 255))
    for mesh in robot_cmd._meshes:
