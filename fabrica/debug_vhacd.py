#!/usr/bin/env python3
"""Drop-test beam part using VHACD convex decomposition collision.

Headless IsaacGym script that drops a part onto the table and captures frames.

Usage:
    python fabrica/debug_vhacd.py --part 2
    python fabrica/debug_vhacd.py --part 6 --num-frames 500
"""
...
def main():
    ...
    parser.add_argument("--output-dir", type=str, default="debug_output/vhacd")
    ...
    # Object asset - VHACD (use IsaacGym's built-in VHACD on original mesh)
    beam_root = f"assets/urdf/fabrica/beam/{args.part}"
    beam_file = f"beam_{args.part}.urdf"
    beam_options = gymapi.AssetOptions()
    beam_options.vhacd_enabled = True
    ...
    print(f"VHACD beam: {gym.get_asset_rigid_body_count(beam_asset)} bodies, "
