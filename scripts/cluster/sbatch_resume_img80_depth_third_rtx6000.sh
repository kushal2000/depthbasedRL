#!/bin/bash
# Resume the active 80x45 third-person depth distillation job from its latest checkpoint.
...
#SBATCH --job-name=img80_depth_third_u1_resume
...
./scripts/run_in_isaacsim_env.sh python isaacsim_conversion/distill.py \
  ...
  --student_checkpoint distillation_runs/img80_depth_third_u1_15209712_2048env/checkpoints/student_latest.pt \
  --num_envs 2048 --env_spacing 4.0 --ground_plane_size 500 --camera_backend tiled \
  --distill_config isaacsim_conversion/configs/hammer_distill_depth_80x45_window_online_dagger_512.yaml \
  --camera_config isaacsim_conversion/configs/hammer_camera_depth_80x45.yaml \
  --teacher_checkpoint pretrained_policy/model.pth --teacher_config pretrained_policy/config.yaml \
  --run_dir "distillation_runs/img80_depth_third_u1_resume_${SLURM_JOB_ID}" \
  --wandb --wandb_entity tylerlum --wandb_project depthbasedRL-isaacsim-distill \
  --wandb_name "img80_depth_third_u1_resume_${SLURM_JOB_ID}" \
