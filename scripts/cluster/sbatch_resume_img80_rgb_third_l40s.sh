#!/bin/bash
# Resume the active 80x45 third-person RGB distillation job from its latest checkpoint.
# Submit manually when ready:
#   sbatch scripts/cluster/sbatch_resume_img80_rgb_third_l40s.sh

#SBATCH --job-name=img80_rgb_third_u1_resume
...
./scripts/run_in_isaacsim_env.sh python isaacsim_conversion/distill.py \
  ...
  --student_checkpoint distillation_runs/img80_rgb_third_u1_15209711_2048env/checkpoints/student_latest.pt \
  ...
  --camera_config isaacsim_conversion/configs/hammer_camera_rgb_80x45.yaml \
  --teacher_checkpoint pretrained_policy/model.pth --teacher_config pretrained_policy/config.yaml \
  --run_dir "distillation_runs/img80_rgb_third_u1_resume_${SLURM_JOB_ID}" \
  --wandb --wandb_entity tylerlum --wandb_project depthbasedRL-isaacsim-distill \
  --wandb_name "img80_rgb_third_u1_resume_${SLURM_JOB_ID}" \
