...
from envs.isaaclab.sim_tool_real_cfg import (
    NUM_ARM_DOFS,
    NUM_FINGERTIPS,
    NUM_HAND_ARM_DOFS,
    NUM_HAND_DOFS,
...
from envs.isaaclab.utils import (
    get_axis_params,
    quat_rotate_wxyz,
    quat_wxyz_to_xyzw,
    quat_xyzw_to_wxyz,
...
class SimToolRealEnv(DirectRLEnv):
    ...
    def _apply_friction_settings(self):
        ...
        self.object.root_physx_view.set_material_properties(object_materials, all_env_ids)
        ...
        print(f"  Robot shapes: {robot_materials.shape[1]}, "
              f"Object shapes: {object_materials.shape[1]}, "
    ...
    def _allocate_buffers(self):
        ...
        self.object_keypoint_offsets = (offsets * base_size * kp_scale / 2).unsqueeze(0).expand(N, -1, -1).clone()

        fixed_size = torch.tensor(self.cfg.fixed_size, device=device, dtype=torch.float)
        self.object_keypoint_offsets_fixed_size = (
            (offsets * fixed_size.unsqueeze(0) * kp_scale / 2).unsqueeze(0).expand(N, -1, -1).clone()
        )
    ...
    def _get_dones(self) -> Tuple[Tensor, Tensor]:
        ...
        if self.cfg.reset_when_dropped:
            ...
            dropped = (object_pos[:, 2] < dropped_z) & self.lifted_object
        else:
            dropped = zeros
    ...
    def _populate_sim_buffers(self):
        ...
        """
        # Lazy-init object mass (available after first scene.update())
        if self._object_rb_masses is None:
            self._object_rb_masses = self.object.data.default_mass.to(self.device)  # (N, 1)
    ...
    def _compute_kuka_reward(self) -> Tensor:
        ...
        # Extras for logging (matches IsaacGym extras format)
        ...
        self.extras["true_objective"] = self.prev_episode_true_objective
    ...
    def _track_reset_reasons(self):
        """Track and log reset reason fractions (matches IsaacGym)."""
        object_pos = self.object.data.root_pos_w
        object_z_low = (object_pos[:, 2] < 0.1).sum().item()

        if self.cfg.max_consecutive_successes > 0:
            max_succ = (self.successes >= self.cfg.max_consecutive_successes).sum().item()
        else:
            max_succ = 0

        max_ep_len = (self.episode_length_buf >= self.max_episode_length - 1).sum().item()

        if hasattr(self, 'curr_fingertip_distances'):
            hand_far = (self.curr_fingertip_distances.max(dim=-1).values > 1.5).sum().item()
        else:
            hand_far = 0

        if self.cfg.reset_when_dropped:
            dropped_z = self.object_init_state[:, 2]
            dropped = ((object_pos[:, 2] < dropped_z) & self.lifted_object).sum().item()
        else:
            dropped = 0
    ...
    def _reset_object_pose(self, env_ids: Tensor):
        """Reset object to start pose with noise (matches IsaacGym reset_object_pose)."""
        N = len(env_ids)
        ...
        # Record canonical position BEFORE noise for lifting reward reference.
        # IsaacGym stores object_init_state without noise, then adds noise to
        # the actual root_state_tensor. z_lift uses init_state as reference.
        ...
        # Convert local position to simulation (world) frame
        pos += self.scene.env_origins[env_ids]
        canonical_pos += self.scene.env_origins[env_ids]

        # Store canonical (no-noise) position as init state (matches IsaacGym)
        quat_xyzw = quat_wxyz_to_xyzw(quat_wxyz)
        self.object_init_state[env_ids, :3] = canonical_pos
        self.object_init_state[env_ids, 3:7] = quat_xyzw
    ...
    def _reset_target_pose(self, env_ids: Tensor, is_first_goal: bool = True):
        ...
        if is_first_goal or self.cfg.goal_sampling_type != "delta":
            min_z = self.object_init_state[env_ids, 2:3] - 0.05 + self.cfg.lifting_bonus_threshold
    ...
    def _apply_random_forces(self):
        ...
        if self.cfg.force_scale > 0.0:
            ...
            if force_mask.any():
                ...
                obj_mass = self._object_rb_masses[force_envs] if self._object_rb_masses is not None else 1.0
        ...
        if self.cfg.torque_scale > 0.0:
            ...
            if torque_mask.any():
                ...
                obj_mass = self._object_rb_masses[torque_envs] if self._object_rb_masses is not None else 1.0
    ...
    def _sample_delta_quat_xyzw(self, input_quat_xyzw: Tensor, delta_degrees: float) -> Tensor:
        ...
        from envs.isaaclab.utils import quat_mul_wxyz
