...
# Hand joint names in URDF depth-first order (must match HAND_STIFFNESSES/DAMPINGS arrays)
HAND_JOINT_NAMES = [
    "left_1_thumb_CMC_FE", "left_thumb_CMC_AA", "left_thumb_MCP_FE",
    "left_thumb_MCP_AA", "left_thumb_IP",                              # thumb (5)
    "left_2_index_MCP_FE", "left_index_MCP_AA", "left_index_PIP",
    "left_index_DIP",                                                  # index (4)
    "left_3_middle_MCP_FE", "left_middle_MCP_AA", "left_middle_PIP",
    "left_middle_DIP",                                                 # middle (4)
    "left_4_ring_MCP_FE", "left_ring_MCP_AA", "left_ring_PIP",
    "left_ring_DIP",                                                   # ring (4)
    "left_5_pinky_CMC", "left_pinky_MCP_FE", "left_pinky_MCP_AA",
    "left_pinky_PIP", "left_pinky_DIP",                                # pinky (5)
]
assert len(HAND_JOINT_NAMES) == NUM_HAND_DOFS

# Per-joint dicts for ImplicitActuatorCfg
HAND_STIFFNESS_DICT = {name: float(s) for name, s in zip(HAND_JOINT_NAMES, HAND_STIFFNESSES)}
HAND_DAMPING_DICT = {name: float(d) for name, d in zip(HAND_JOINT_NAMES, HAND_DAMPINGS)}
HAND_ARMATURE_DICT = {name: float(a) for name, a in zip(HAND_JOINT_NAMES, HAND_ARMATURES)}
HAND_FRICTION_DICT = {name: float(f) for name, f in zip(HAND_JOINT_NAMES, HAND_FRICTIONS)}
...
class SimToolRealEnvCfg(DirectRLEnvCfg):
    ...
    robot_cfg: ArticulationCfg = ArticulationCfg(
        ...
        actuators={
            ...
            "sharpa_hand": ImplicitActuatorCfg(
                ...
                stiffness=HAND_STIFFNESS_DICT,
                damping=HAND_DAMPING_DICT,
                armature=HAND_ARMATURE_DICT,
                friction=HAND_FRICTION_DICT,
            ),
        },
    )
    ...
    tiled_camera: TiledCameraCfg = TiledCameraCfg(
        ...
        offset=TiledCameraCfg.OffsetCfg(
            pos=(-0.11, 0.00, -0.03),
            rot=(0.5018, 0.3481, -0.5893, 0.5289),  # wxyz: rot37 tx-5 ty-10 from base
            convention="opengl",
        ),
        data_types=["depth"],
    ...
    # ── Reset conditions ──
    reset_when_dropped: bool = False  # matches IsaacGym default (resetWhenDropped: False)
    ...
    force_consecutive_near_goal_steps: bool = True
    ...
    # ── Object scale noise ──
    object_scale_noise_multiplier_range: tuple = (0.9, 1.1)

    # ── Video capture (matches IsaacGym capture_video settings) ──
    capture_video: bool = False
    capture_video_freq: int = 1464  # steps between video recordings
    capture_video_len: int = 100    # frames per video clip
    ...
    init_tyler_curriculum_scale: float = 0.0
    time_to_update_tyler_curriculum: float = 5.0  # minutes
    update_step_size_tyler_curriculum: float = 0.01
