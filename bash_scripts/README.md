...
48. `48_ffs_env_import_smoke.sh`
    Checks the isolated `ffs_isaacsim_ros_py311` conda env can import ROS, IsaacLab/IsaacSim, OpenCV, Torch, and Fast-FoundationStereo dependencies.

49. `49_ffs_demo_smoke.sh`
    Runs Fast-FoundationStereo's bundled demo through the isolated env and writes outputs to `local_logs/ffs_demo_smoke`.

50. `50_isaac_ffs_stereo_benchmark.sh`
    IsaacSim benchmark using rendered left/right RGB cameras plus Fast-FoundationStereo depth instead of Isaac's direct metric depth. Saves stereo/depth debug frames under `local_logs/ffs_isaac_stereo_debug` by default.

51. `51_isaac_ffs_stereo_ros_node.sh`
    ROS source node version of 50. It publishes FFS-estimated metric depth on the same ZED depth topic used by the student policy node.

60. `60_depth_debug_local_fake_pipeline_tmux.sh [checkpoint]`
    Starts the full localhost-only fake pipeline in a tmux session: roscore,
    fake robot, fake depth from `fake_depth_0p85m_160x90.npz`, Viser, and the
    student policy in dry-run recording mode. Override `DEPTH_FILE`,
    `RUN_DURATION_S`, `RECORD_DIR`, or `SESSION` as needed. Requires `tmux`.
    If the default fake depth file is missing, the script creates a constant
    0.85 m metric depth NPZ.

61. `61_depth_debug_local_isaac_pipeline_tmux.sh [checkpoint]`
    Starts the full localhost-only IsaacSim-backed pipeline in tmux: roscore,
    an IsaacSim depth source node, Viser, and the student policy in dry-run
    recording mode. Defaults to clean fixed-camera IsaacSim depth via script 45.
    Override `ISAAC_DEPTH_SCRIPT` with scripts 42, 43, 44, 45, 46, or 47.
    Requires `tmux`.

62. `62_ws16_direct_zed_log_only.sh [checkpoint]`
    Direct-ZED ws-16/ws-2 style log-only run. It sets `ROS_MASTER_URI` to
    `http://bohg-ws-2.stanford.edu:11311` by default, reads depth directly from
    the ZED SDK with the nonblocking shared-memory producer, does not subscribe
    to a ROS depth image topic, does not publish joint commands, and records a
    rollout NPZ to `./student_depth_zed_direct_recording`.

63. `63_visualize_latest_student_depth_recording.sh [recording_or_dir]`
    Opens a recorded student-depth rollout in Viser. Pass a specific `.npz`, a
...
Fast-FoundationStereo IsaacSim pipeline:

1. `bash_scripts/48_ffs_env_import_smoke.sh`
2. `bash_scripts/49_ffs_demo_smoke.sh`
3. `bash_scripts/50_isaac_ffs_stereo_benchmark.sh`
4. Terminal A: `bash_scripts/30_depth_debug_roscore_local.sh`
5. Terminal B: `DEPTH_EVERY_N=2 bash_scripts/51_isaac_ffs_stereo_ros_node.sh`
6. Terminal C: `bash_scripts/35_depth_debug_visualization_with_depth.sh`
7. Terminal D: `RUN_DURATION_S=30 bash_scripts/33_depth_debug_student_ros_topic_dry_run.sh`

The FFS path is opt-in via `--depth_render_backend ffs_stereo`; the default IsaacSim node still publishes direct Isaac metric depth. The current synthetic stereo baseline default is `FFS_BASELINE_M=0.12`, matching an approximate ZED1 baseline. The right stereo camera is generated by translating the left student camera along its local +X axis, so the same camera-pose randomization applies to both cameras as a rigid stereo pair.

One-command tmux wrappers:

- Fake robot/depth local stack:
  `bash_scripts/60_depth_debug_local_fake_pipeline_tmux.sh`
- IsaacSim clean-depth local stack:
  `bash_scripts/61_depth_debug_local_isaac_pipeline_tmux.sh`
- IsaacSim train-settings local stack:
